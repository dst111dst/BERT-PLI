...
def test(model, dataset, epoch, writer, config, gpu_list, output_function, mode="test"):
    model.eval()
    acc_result = None
    total_loss = 0
    cnt = 0
    ...
    micro_prec_query, micro_recall_query, micro_f1_query = eval_micro_query(result)
    loss_tmp = total_loss / (step + 1)
    print('test set: micro_prec_query=%.4f, micro_recall_query=%.4f, micro_f1_query=%.4f' %
          (micro_prec_query, micro_recall_query, micro_f1_query))
